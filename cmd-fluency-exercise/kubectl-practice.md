...
- Create a pod manifest for nginx  image with requests and limits and run a sleep command </br> $ k run nginx $do --image=nginx --requests "cpu=100m,memory=256Mi" --limits "cpu200m,memory=512Mi" --command -- sh -c "sleep 300"
